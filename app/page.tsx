...
export default function Home() {
  ...
  /** Verifica os campos obrigatórios dos grupos 1 a 4. Retorna a lista de
   * campos que ainda faltam preencher (vazia se estiver tudo ok). */
  function camposObrigatoriosFaltando(): string[] {
    const faltando: string[] = [];

    // Grupo 1 – Ente contratante
    if (!nomeEnte.trim()) faltando.push("Nome do ente");
    if (!uf.trim()) faltando.push("UF");

    // Grupo 2 – Data de emissão
    if (!dataDia || !dataMes || !dataAno) faltando.push("Data de emissão");

    // Grupo 3 – Destinatário
    if (!nomeDestinatario.trim()) faltando.push("Nome do destinatário");
    if (!cargoDestinatario.trim()) faltando.push("Cargo do destinatário");

    // Grupo 4 – Prazo e honorários
      ...
      <div className="page">
        <section className="card">
          ...
          <div className="grid">
            ...
            <div className="field">
              <label>UF *</label>
              <input value={uf} onChange={(e) => setUf(e.target.value.toUpperCase())} maxLength={2} placeholder="Ex.: SP" />
            </div>
          </div>
        </section>
